...
A.🐳 Docker Compose 部署指南
你需要创建两个必要的文件：docker-compose.yml 和 .env
...
services:
  hot-apis:
    ...
    restart: always
    ports:
      - "8000:8000"  # 宿主机端口:容器内端口，左侧的8000可根据需要修改
    volumes:
      # 挂载本地配置文件，保证重新拉取镜像后数据不丢失
      - ./.env:/app/.env
    environment:
      - TZ=Asia/Shanghai
-------------------------------------------------
⚠️ 注意：在启动前，必须确保当前目录下存在 .env 文件，否则 Docker 可能错误。
